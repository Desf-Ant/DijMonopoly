...
/*void DrawCard::Draw(typeDrawCard typeDraw){
    ...
    if (type == typeDrawCard::ComCard){
        ...
        std::string pioche1 = ComCard [ (int)(rand()*ComCard.size()) ];
    }
    ...
    if (type == typeDrawCard::ChanceCard){
        ...
        std::string pioche2 = ChanceCard [ (int)(rand()*ChanceCard.size()) ];
    }
}
*/

std::string DrawCard::RecupFichiercom(){
    QFile filecom ("Z:\Documents\Monopoly\DijMonopoly\DijonMonopoly\cartescom.txt");
    ...
    while (!in.atEnd()){
        ...
        ComCard.push_back(line.toStdString());
...
std::string DrawCard::RecupFichierchance(){
    QFile filechance ("Z:\Documents\Monopoly\DijMonopoly\DijonMonopoly\carteschance.txt");
    ...
    while (!in.atEnd()){
        QString line = in.readLine();
        ChanceCard.push_back(line.toStdString());
        // std::cout << line.toStdString() << std::endl;
    }
...
std::string DrawCard::DrawChanceCard(){
    ...
    std::string piocheChance = ChanceCard [ (int)(rand()*ChanceCard.size()) ];
}
